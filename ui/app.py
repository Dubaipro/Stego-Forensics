...
def sign_message(private_key_obj, message):
...
def send_authority_alert(record):
    ...
    msg = Message(
        subject="🚨 Tampering Alert - StegoForensics",
        sender=app.config['MAIL_USERNAME'],
...
⚠️ The image failed authenticity verification.
...
app.config['SQLALCHEMY_DATABASE_URI'] = (
    "postgresql+psycopg2://stego_forensics_db_user:54gE6lMFpT5YR9SWIKdVLAOq3Gh9o0EE"
    "@dpg-d01tjjbuibrs73b5ujj0-a.oregon-postgres.render.com/stego_forensics_db"
    "?sslmode=require"
)
...
mail = Mail(app)
# Encryption key from environment variable (secure and not hardcoded)
fernet = Fernet(os.environ.get("FERNET_SECRET_KEY").encode())
...
class User(UserMixin, db.Model):
    ...
    pending_approval = db.Column(db.Boolean, default=True)  # ✅ Default False until admin approval
    verification_code = db.Column(db.String(6), nullable=True)
    two_factor_code = db.Column(db.String(6), nullable=True)
    code_sent_at = db.Column(db.DateTime, nullable=True)
    private_key = db.Column(db.Text, nullable=True)  # encrypted
    public_key = db.Column(db.Text, nullable=True)  # not encrypted
...
def role_login(role):
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password, password):
            ...
            if user.role != role:
                flash(f"🚫 You are not authorized as a {role.capitalize()}.", 'error')
                return redirect(url_for('role_login', role=role))
            ...
            flash("📨 2FA code sent to your email. Please verify to continue.", "info")
...
def verification_trends():
    ...
    # 🔥 Filter out dates that have zero activity
...
def approve_user(token):
    ...
    try:
        ...
        flash("❌ Invalid or expired approval token.", "error")
    return redirect(url_for('dashboard'))
...
def reject_user(token):
    ...
    try:
        ...
        if user and action == 'reject':
            ...
            db.session.commit()
            flash(f"❌ User '{user.username}' was rejected and removed.", "info")
    except:
        flash("❌ Invalid or expired rejection token.", "error")
    return redirect(url_for('dashboard'))
    ...
    for record in records:
        ...
        try:
            if os.path.exists(image_path):
                ...
                if not is_valid:
                    ...
                    if not TamperLog.query.filter_by(record_id=record.id).first():
                        tamper = TamperLog(
                            record_id=record.id,
                            discrepancy_details=f"⚠️ Signature mismatch for image '{record.filename}' during dashboard check.",
                            notified=False,
            ...
            if not TamperLog.query.filter_by(record_id=record.id).first():
                tamper = TamperLog(
                    record_id=record.id,
                    discrepancy_details=f"❌ Error verifying: {str(e)}",
                    notified=False,
                    timestamp=datetime.utcnow()
                )
                db.session.add(tamper)
...
def edit_metadata(record_id):
    ...
    # 🛑 Prevent editing if the image is marked as tampered
    if not record.verification:
        flash("🚫 You cannot edit metadata for a tampered image. Please re-verify it first.", "error")
...
Notes: {new_notes}"""
        ...
        flash("✅ Metadata updated successfully and re-embedded in the image.", "success")
        return redirect(url_for('view_metadata', record_id=record.id))
...
def update_user(user_id):
    ...
    if not username or not email:
        flash("⚠️ Username and Email are required.", "error")
...
def download_metadata(record_id):
    ...
    if current_user.role == 'legal' and not record.verification:
        flash("❌ You are only allowed to download verified reports.", "error")
        return redirect(url_for('legal_dashboard'))
...
def download_file(filename):
    ...
    if current_user.role == 'legal':
        ...
        if record and not record.verification:
            flash("🚫 You are only allowed to download verified images.", "error")
            return redirect(url_for('legal_dashboard'))
...
def view_metadata(record_id):
    ...
    if current_user.role == 'investigator' and record.user_id != current_user.id:
        flash("🚫 You can only view your own forensic records.", "error")
...
def add_user_by_admin():
    if request.method == 'POST':
        ...
        new_user = User(
            ...
            pending_approval=False          # ✅ Explicitly not pending
        )
...
def verify_again(record_id):
    ...
    try:
        ...
        if extracted == "Missing" or not extracted:
            flash("❌ Could not extract metadata for verification.", "error")
            return redirect(request.referrer or url_for('dashboard'))
        ...
        if is_valid:
            # ✅ Clear all tamper logs if valid again
            TamperLog.query.filter_by(record_id=record.id).delete()
            flash("✅ Re-verified successfully. Marked as Authentic.", "success")
        else:
            flash("❌ Image still Tampered after verification attempt.", "error")
...
def view_tamper_logs():
    ...
    if not logs:
        flash("✅ No tampering activity has been logged yet.", "info")
...
def upload():
    if current_user.role == 'legal':
        flash("🚫 Legal authorities cannot upload images.", "error")
        return redirect(url_for('legal_dashboard'))
    ...
    if request.method == 'POST':
        ...
        if not re.fullmatch(r'\d{4}', case_id):
            flash("❌ Case ID must be exactly 4 digits (e.g., 1234).", "error")
...
Notes: {notes}"""
                ...
                if img_file and '.' in img_file.filename:
                    ...
                    try:
                        if "Case ID:" in extract_data(temp_path):
                            flash("🚫 One of the images already contains embedded metadata.", "error")
                            os.remove(temp_path)
                            return redirect(url_for('upload'))
                    except:
                        pass
        ...
        if step == 'confirm':
            if not image_filenames:
                flash("❌ Image not found. Please re-upload and preview.", "error")
                return redirect(url_for('upload'))

            if ForensicRecord.query.filter_by(case_id=case_id).first():
                flash(f"⚠️ Metadata for Case ID {case_id} already exists.", "warning")
                return redirect(url_for('upload'))

            try:
                timestamp = datetime.strptime(timestamp_str, '%Y-%m-%dT%H:%M')
            except:
                flash("⚠️ Invalid timestamp. Using current time.", "warning")
                timestamp = datetime.utcnow()

            if timestamp > datetime.now():
                flash("⚠️ Timestamp cannot be in the future.", "error")
                return redirect(url_for('upload'))

            # 🔐 Decrypt current user's private key
            try:
                decrypted_key = fernet.decrypt(current_user.private_key.encode()).decode()
                private_key_obj = RSA.import_key(decrypted_key)
            except Exception as e:
                flash(f"❌ Could not decrypt private key: {str(e)}", "error")
                return redirect(url_for('upload'))
            ...
            for filename in image_filenames.split(','):
                ...
                signature = sign_message(private_key_obj, extracted).hex()
            ...
            db.session.commit()
            flash("✅ All images embedded successfully!", "success")
            return render_template("upload.html",
...
def stored_images():
    ...
    for record in paginated.items:
        ...
        # 🚨 Force status to "Tampered" if tamper log exists
...
def extract():
    if current_user.role == 'legal':
        flash("🚫 Legal authorities cannot extract metadata manually.", "error")
        return redirect(url_for('legal_dashboard'))
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if not embedded_case_id:
                flash("⚠️ Could not extract Case ID from metadata.", "error")
            ...
            if embedded_case_id != case_id:
                flash(f"❌ Case ID mismatch. The embedded ID is '{embedded_case_id}', but you entered '{case_id}'.", "error")
                return render_template("extract.html", extracted_metadata=extracted_metadata, verification=False,
                                       signature=None, filename=filename, case_id=case_id)

            record = ForensicRecord.query.filter_by(case_id=case_id).first()
            if not record:
                flash("❌ No forensic record found for this Case ID.", "error")
                return render_template("extract.html", extracted_metadata=extracted_metadata, verification=False,
                                       signature=None, filename=filename, case_id=case_id)

            # Restrict investigators from extracting others' records
            if current_user.role == 'investigator' and record.user_id != current_user.id:
                flash("🚫 You are only allowed to extract your own forensic records.", "error")
                return redirect(url_for('extract'))
            ...
            if os.path.exists(original_path):
                ...
                if uploaded_hash != original_hash:
                    if not TamperLog.query.filter_by(record_id=record.id, discrepancy_details="⚠️ Image file hash mismatch: content has been altered.").first():
                        tamper = TamperLog(
                            record_id=record.id,
                            discrepancy_details="⚠️ Image file hash mismatch: content has been altered.",
                            notified=False
                        )
                        db.session.add(tamper)
                    ...
                    flash("🚨 Image file has been tampered. Tampering logged.", "error")
                    return redirect(url_for('view_tamper_logs'))
            else:
                flash("⚠️ Original file missing. Cannot verify hash.", "warning")
            ...
            if is_verified:
                ...
                db.session.commit()
                flash("✅ Metadata extracted and verified successfully.", "success")
            else:
                if not TamperLog.query.filter_by(record_id=record.id, discrepancy_details="⚠️ Digital signature mismatch detected.").first():
                    tamper = TamperLog(
                        record_id=record.id,
                        discrepancy_details="⚠️ Digital signature mismatch detected.",
                        notified=False
                    )
                    db.session.add(tamper)
                ...
                db.session.commit()
                send_authority_alert(record)
                flash("🚨 Metadata has been modified. Tampering logged.", "error")
                return redirect(url_for('view_tamper_logs'))
            ...
            if record:
                if not TamperLog.query.filter_by(record_id=record.id, discrepancy_details=str(e)).first():
                    try:
                        tamper = TamperLog(
                            record_id=record.id,
                            discrepancy_details=f"❌ Metadata extraction failed: {str(e)}",
                            notified=False
                        )
                        db.session.add(tamper)
                        db.session.commit()
                        ...
                        db.session.rollback()
                        flash(f"❌ Tamper log save failed: {inner_err}", "error")
...
def register():
    if request.method == 'POST':
        ...
        if not validate_email(email):
            flash("❌ Please enter a valid and reachable email address.", "error")
            return render_template('register.html')
        ...
        if len(password) < 8 or not any(c.isdigit() for c in password) or \
           not any(c.isupper() for c in password) or not re.search(r'[!@#$%^&*(),.?\":{}|<>]', password):
            flash("❌ Password must include uppercase, number, and special character.", "error")
            return render_template('register.html')
        ...
        raw_private_key = user_key.export_key().decode('utf-8')
        encrypted_private_key = fernet.encrypt(raw_private_key.encode()).decode()
...
def delete_record(record_id):
    if current_user.role == 'legal':
        flash("🚫 Legal authorities cannot delete records.", "error")
        return redirect(url_for('legal_dashboard'))

    record = ForensicRecord.query.get_or_404(record_id)

    # 🛡️ Safely delete associated tamper logs first
    TamperLog.query.filter_by(record_id=record.id).delete()

    db.session.delete(record)
    db.session.commit()

    flash('✅ Record and associated tamper logs deleted successfully.', 'success')
    return redirect(url_for('dashboard'))
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password, password):
            ...
            if selected_role != user.role:
                flash("❌ Role mismatch. Please check your selected role.", "error")
                return redirect(url_for('login'))
...
def logout():
    logout_user()
    session.pop('2fa_verified', None)  # ✅ Clear 2FA session
